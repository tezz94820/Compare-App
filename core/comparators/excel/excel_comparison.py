...
def compare_excel_page_worker(page_num, dev_rows, prod_rows):
    ...
    if dev_rows or prod_rows:
        dev_content = "\n".join(dev_rows)
        prod_content = "\n".join(prod_rows)
        page_similarity = difflib.SequenceMatcher(None, dev_content, prod_content).ratio()
    else:
        page_similarity = 1.0

    return (page_num, diff, added, removed, changed, unchanged, page_similarity, dev_rows, prod_rows)
    ...
    def compare_sheets_pagewise(self):
        ...
        for sheet_name in sorted(all_sheet_names):
            ...
            for page_num in sorted(results.keys()):
                ...
                page_similarity, dev_rows, prod_rows) = results[page_num]
    ...
    def calculate_analytics(self) -> Dict:
        ...
        for sheet in self.sheet_page_diffs:
            for page in sheet["pages"]:
                ...
                total_page_similarity += page["stats"]["page_similarity"]
                page_count += 1
